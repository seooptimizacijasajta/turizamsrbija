...
export type Business = {
  ...
  image: string | null; featured: boolean; paid: boolean;
};
function row(r: any): Business {
  const active = !r.featured_until || new Date(r.featured_until) >= new Date(new Date().toDateString());
  const paidActive = !r.paid_until || new Date(r.paid_until) >= new Date(new Date().toDateString());
  return {
    ...
    image: r.image || null, featured: !!r.featured && active, paid: !!r.paid && paidActive,
  };
}
export async function getBusinesses(): Promise<Business[]> {
  const sb = getServerClient(); if (!sb) return [];
  const { data } = await sb.from("businesses").select("*").eq("status", "approved").order("featured", { ascending: false }).order("paid", { ascending: false }).order("created_at", { ascending: false });
  return (data || []).map(row);
}
