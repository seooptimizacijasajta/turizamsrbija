...
export async function generateMetadata({ params }: { params: Promise<{ tip: string }> }) {
  ...
  return { title: `${st.sr} apartmani Beograd — stan na dan | Turizam Srbija`, description: `${st.sr} apartmani i stan na dan u Beogradu. Proveren smeštaj sa fotografijama, mapom i direktnim kontaktom vlasnika, bez provizije za gosta.`, alternates: { canonical: `/apartmani-beograd/struktura/${tip}`, languages: { "sr-Latn-RS": `/apartmani-beograd/struktura/${tip}`, en: `/en/belgrade-apartments/type/${tip}`, de: `/de/belgrad-apartments/typ/${tip}`, "x-default": `/apartmani-beograd/struktura/${tip}` } } };
}
