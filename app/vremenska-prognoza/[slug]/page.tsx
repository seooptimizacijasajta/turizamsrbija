...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const name = wxName(p, 'sr');
  const title = 'Vremenska prognoza {name} — 7 dana | Turizam Srbija'.replace("{name}", name);
  const description = 'Vremenska prognoza za {name}: trenutna temperatura, prognoza za narednih 7 dana, padavine i vetar, uz predlog smeštaja u blizini.'.replace("{name}", name);
  return {
    title, description,
    alternates: {
      canonical: `/vremenska-prognoza/${slug}`,
