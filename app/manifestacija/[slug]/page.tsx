...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return { title: `${ev.name}${ev.city ? " — " + ev.city : ""} | Turizam Srbija`, description: (ev.desc.sr || ev.name).slice(0, 160), alternates: { canonical: `/manifestacija/${slug}`, languages: { "sr-Latn-RS": `/manifestacija/${slug}`, en: `/en/event/${slug}`, de: `/de/veranstaltung/${slug}`, "x-default": `/manifestacija/${slug}` } } };
}
