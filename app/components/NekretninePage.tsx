...
import { PROP_TYPES, propTypeByKey, propTypeLabel, propTypePath, propertyPath, dealKindLabel, nekretnineIndexPath } from "@/lib/nekretnine";
...
export default function NekretninePage({ properties, type = "" }: { properties: Property[]; type?: string }) {
  ...
  const lead = lang === "sr" ? "Prodaja i izdavanje: stanovi, kuće, placevi, seoska domaćinstva i poslovni prostor širom Srbije."
    : lang === "de" ? "Verkauf und Vermietung: Wohnungen, Häuser, Grundstücke, Landgüter und Gewerbe in ganz Serbien."
    : "For sale and rent: apartments, houses, land, village estates and commercial property across Serbia.";

  const ld = list.length ? {
    "@context": "https://schema.org", "@type": "ItemList",
    itemListElement: list.slice(0, 50).map((p, i) => ({ "@type": "ListItem", position: i + 1, url: "https://turizamsrbija.com" + propertyPath(p.id, lang), name: p.title })),
  } : null;
    ...
    <>
      ...
      <div className="container" style={{ paddingBottom: 50 }}>
        ...
        <div className="amen-filter">
          <button type="button" className={"amen-chip" + (deal === "" ? " on" : "")} onClick={() => setDeal("")}>{lang === "sr" ? "Sve" : lang === "de" ? "Alle" : "All"}</button>
          <button type="button" className={"amen-chip" + (deal === "prodaja" ? " on" : "")} onClick={() => setDeal("prodaja")}>{dealKindLabel("prodaja", lang)}</button>
          <button type="button" className={"amen-chip" + (deal === "najam" ? " on" : "")} onClick={() => setDeal("najam")}>{dealKindLabel("najam", lang)}</button>
        </div>
        {!cat && (
          <div className="amen-filter">
            <button type="button" className={"amen-chip" + (ptype === "" ? " on" : "")} onClick={() => setPtype("")}>{lang === "sr" ? "Svi tipovi" : lang === "de" ? "Alle Typen" : "All types"}</button>
            {PROP_TYPES.map((c) => (
        ...
        {list.length ? (<>
          <div className="card-grid" style={{ marginTop: 18 }}>
            {list.slice(0, shown).map((p) => {
              ...
              return (
                <Link href={propertyPath(p.id, lang)} className={"card" + (p.bold ? " card--bold" : "")} key={p.id} style={{ display: "block" }}>
                  <div className="card-media" style={p.image ? undefined : { background: "linear-gradient(135deg,#0f3d2e,#176b4e)", display: "grid", placeItems: "center", minHeight: 150 }}>
                    ...
                    <span className="card-badge">{dealKindLabel(p.deal_type, lang)}{pc ? " · " + propTypeLabel(pc, lang) : ""}</span>
                  </div>
                  <div className="card-body">
                    <span className="card-region">{[p.city, p.municipality].filter(Boolean).join(" · ")}</span>
                    <h3 className="card-title">{p.title}</h3>
                    {specs && <p className="card-desc">{specs}</p>}
                    <div className="card-foot">{p.price != null && <span className="price">{eur(p.price)}{p.deal_type === "najam" ? <small> / {lang === "sr" ? "mesec" : lang === "de" ? "Monat" : "month"}</small> : null}</span>}</div>
                  </div>
                </Link>
              );
            })}
