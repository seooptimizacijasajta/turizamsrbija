...
export default function SavedView({ items }: { items: Listing[] }) {
  ...
  return (
    <div className="container" style={{ padding: "40px 0" }}>
      <h1 style={{ marginBottom: 18 }}>{lang !== "sr" ? "Saved" : "Sačuvano"} ♥</h1>
      {saved.length ? <div className="card-grid">{saved.map((d) => <ListingCard key={d.id} item={d} />)}</div>
        : <div className="empty">{lang !== "sr" ? "No saved items yet. Tap ♥ on a listing." : "Još nema sačuvanih. Klikni ♥ na nekom smeštaju."}</div>}
    </div>
  );
}
