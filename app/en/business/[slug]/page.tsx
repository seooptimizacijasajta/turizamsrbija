...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!b) return { title: "Business — Turizam Srbija" };
  return { title: `${b.name}${b.city ? " — " + b.city : ""} | Turizam Srbija`, description: (b.desc.en || b.desc.sr || b.name).slice(0, 160), openGraph: { images: [b.image || `/api/og?title=${encodeURIComponent(b.name)}`] }, alternates: { canonical: `/en/business/${slug}`, languages: { "sr-Latn-RS": `/firma/${slug}`, en: `/en/business/${slug}`, de: `/de/firma/${slug}`, "x-default": `/firma/${slug}` } } };
}
