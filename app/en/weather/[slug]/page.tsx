...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const name = wxName(p, 'en');
  const title = '{name} weather forecast — 7 days | Turizam Srbija'.replace("{name}", name);
  const description = 'Weather forecast for {name}: current temperature, the next seven days, precipitation and wind, plus nearby accommodation.'.replace("{name}", name);
  return {
    title, description,
    alternates: {
      canonical: `/en/weather/${slug}`,
